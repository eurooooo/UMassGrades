...
export default function SearchWithDropdown({ isHome = false }) {
  ...
  const [searchResults, setSearchResults] = useState({ classes: [], professors: [] })
  ...
  const debouncedSearch = debounce((query) => {
    ...
    setSearchResults({ classes: filteredClasses, professors: filteredProfessors })
  }, 300)
  ...
  return (
    <div className={`relative ${isHome ? "max-w-2xl mx-auto" : ""}`} ref={dropdownRef}>
      ...
      />
      {isDropdownOpen && (searchResults.classes.length > 0 || searchResults.professors.length > 0) && (
        <div className="absolute z-10 w-full mt-1 bg-white border rounded-md shadow-lg">
          {searchResults.classes.length > 0 && (
            <div className="p-2">
              <h3 className="mb-2 text-sm font-semibold text-gray-500">Classes</h3>
          ...
          {searchResults.professors.length > 0 && (
            <div className="p-2 border-t">
              <h3 className="mb-2 text-sm font-semibold text-gray-500">Professors</h3>
