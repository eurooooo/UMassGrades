...
export function formatGradeData(grades) {
  // Calculate the total number of students
  const totalStudents = grades.reduce((sum, { count }) => sum + count, 0);
  ...
  return grades
    ...
    .map(({ grade, count }) => ({
      grade,
      students: count,
      value: Number(((count / totalStudents) * 100).toFixed(1)), // Calculate percentage and format to 1 decimal
      color: gradeToColor(grade),
    }));
}

export function mergeGrades(profs) {
  const mergedGrades = {};

  profs.forEach((prof) => {
    prof.grades.forEach(({ grade, count }) => {
      if (!mergedGrades[grade]) {
        mergedGrades[grade] = 0; // Initialize grade if not present
      }
      mergedGrades[grade] += count; // Add count to the grade
    });
  });

  // Convert mergedGrades object into an array of objects
  return Object.entries(mergedGrades).map(([grade, count]) => ({
    grade,
    count,
  }));
}
